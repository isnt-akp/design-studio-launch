...
import { X, Receipt, ArrowUpRight, ArrowDownLeft, RotateCcw, CreditCard, Camera } from "lucide-react";
...
const FOREX_CC_MARKUP = 3.5; // Standard CC forex markup %
...
export function TransactionForm({ onClose }: { onClose: () => void }) {
  ...
  // Compute final INR amount with forex markup for CC (Scenario 1.15)
  const computeFinalAmount = (): number => {
    const amountNum = parseFloat(amount);
    if (!amountNum) return 0;
    if (currency === "INR") return amountNum;
    
    const rate = parseFloat(forexRate);
    if (!rate) return amountNum;
    
    const baseINR = amountNum * rate;
    // Apply CC forex markup if paying by credit card
    const isCreditCard = paymentMethod === "Credit Card";
    const markup = isCreditCard ? baseINR * (FOREX_CC_MARKUP / 100) : 0;
    return Math.round(baseINR + markup);
  };
  ...
  const handleSave = () => {
    ...
    addTransaction({
      ...
      forexMarkup: currency !== "INR" && paymentMethod === "Credit Card" ? FOREX_CC_MARKUP : undefined,
  ...
  const handleReceiptData = (data: { merchant: string; amount: string; category: string }) => {
    setSubCategory(data.merchant);
  ...
  return (
    <>
      {showScanner && <ReceiptScanner onExtracted={handleReceiptData} onClose={() => setShowScanner(false)} />}
        ...
        <div className="relative w-full max-w-lg max-h-[90vh] overflow-y-auto glass-card rounded-card p-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setShowScanner(true)}
                className="h-8 px-3 rounded-button bg-muted text-muted-foreground text-xs flex items-center gap-1 hover:text-foreground transition-colors">
          ...
          {type === "Transfer" && (
            <div className="space-y-2">
              ...
              {/* CC bill payment hint */}
              {accounts.find(a => a.id === toAccountId)?.type === "credit_card" && (
                <p className="text-[10px] text-accent-light">💡 CC bill payment: reduces both savings & CC liability</p>
              )}
            </div>
          )}
            ...
            {currency !== "INR" && (
              <div className="space-y-1.5">
                <input type="number" value={forexRate} onChange={(e) => setForexRate(e.target.value)}
                  placeholder="Forex rate (₹ per unit)" className="w-full h-8 px-3 text-sm bg-muted rounded-button text-foreground font-mono outline-none placeholder:text-muted-foreground" />
                {forexRate && amount && (
                  <div className="p-2 bg-muted/50 rounded-button text-xs space-y-0.5">
                    <div className="flex justify-between text-muted-foreground">
                      <span>Base conversion</span>
                      <span className="font-mono">₹{(parseFloat(amount) * parseFloat(forexRate)).toLocaleString("en-IN")}</span>
                    </div>
                    {paymentMethod === "Credit Card" && (
                      <div className="flex justify-between text-warning">
                        <span>CC forex markup ({FOREX_CC_MARKUP}%)</span>
                        <span className="font-mono">+₹{Math.round(parseFloat(amount) * parseFloat(forexRate) * FOREX_CC_MARKUP / 100).toLocaleString("en-IN")}</span>
                      </div>
                    )}
                    <div className="flex justify-between text-foreground font-semibold border-t border-border pt-1">
                      <span>Final debit</span>
                      <span className="font-mono">₹{computeFinalAmount().toLocaleString("en-IN")}</span>
                    </div>
                  </div>
                )}
              </div>
